...
//     REQUIRE(myFinder.getEnd() == Coord(14, 0)); //look at image and see this is the closest square
// }
...
TEST_CASE("Checkoff test case", "[pathfinder]")
{
    Image<Pixel> img(8,8);
    ...
    myFinder.findPath();
    REQUIRE(myFinder.getEnd() == Coord(0,1));
    for (int i = 0; i < 8; i++)
    {
        for (int j = 0; j < 8; j++)
        {
            REQUIRE(img(i,j) != YELLOW);
    ...
    myFinder.load(img);
    myFinder.findPath();
    REQUIRE(myFinder.getEnd() == Coord(0,2));
    for (int i = 0; i < 8; i++)
    {
        for (int j = 0; j < 8; j++)
        {
...
TEST_CASE("checkoff function test 2", "[pathfinder]")
{
    Image<Pixel> img = readFromFile("../tests/maze00.png");
    img(14,2) = BLACK;
    PathFinder myFinder(img);
    myFinder.findPath("NESW");
    REQUIRE(myFinder.getNumofDeadEnds() == 1);
}

TEST_CASE("checkoff function test 3", "[pathfinder]")
{
    Image<Pixel> img = readFromFile("../tests/maze01.png");
    PathFinder myFinder(img);
    REQUIRE_THROWS_AS(myFinder.findPath("NESW"), std::runtime_error);
    REQUIRE(myFinder.getNumofDeadEnds() == 371);
}