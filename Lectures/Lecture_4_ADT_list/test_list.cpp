...
//    for (int i = 0; i < 6; i++)
//    {
//       REQUIRE(list.insert(i, items[i]));
//    }

//    LinkedList<std::string> copyOfList;
//    copyOfList = list; // invoke copy assignment operator

//    REQUIRE(copyOfList.getLength() == list.getLength());
//    for (int i = 0; i < copyOfList.getLength(); i++)
//    {
//       REQUIRE(copyOfList.getEntry(i) == list.getEntry(i));
//    }
...
//    do
//    {
//       std::cout << "Enter A for ArrayList or L for LinkedList: ";
//       std::cin >> choice;
...
//    listPtr->insert(1, "one"); // put the string one in position one
//    listPtr->getEntry(1);   //get entry at one
//    REQUIRE(listPtr->isEmpty() == false);     //return true if empty but shouldn't be?
//    listPtr->getLength();   //return the number of entries, should be one?
//    listPtr->replace(1, "ONE");   //replace "one" with "ONE" at position one
//    listPtr->remove(1);  //remove at position one
//    listPtr->clear();    //remove all positions
// }

TEST_CASE("exam review question 5", "[List]")
{
   ArrayList<int> studentList;

   studentList.insert(1, 72);
   studentList.insert(2, 91);
   studentList.insert(1, 45);
   studentList.insert(4, 88);
   studentList.replace(2, 8); //changed set entry to replace
   studentList.remove(1);
   studentList.insert(studentList.getLength(), 33);
   int result = studentList.getEntry(2);
   studentList.insert(3, result);
   
   studentList.remove(8);
   studentList.insert(1, 99);

   REQUIRE(studentList.getEntry(1) == 8);
   REQUIRE(studentList.getEntry(2) == 91);
   REQUIRE(studentList.getEntry(3) == 91);
   REQUIRE(studentList.getEntry(4) == 33);
   REQUIRE(studentList.getEntry(5) == 88);
   REQUIRE(studentList.getEntry(6) == 72);
}